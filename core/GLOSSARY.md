...
is closed. `rig story verify <id>` executes a story's `verify_command`,
`rig story verify-all` runs all configured story proof commands, and
`rig trace --story <id>` warns when that story's verification has not passed.

## Tool Registry

The compiled and registered tool manifest exposed by
`rig query tools`. It lets agents discover available commands, arguments,
...
The advisory result from `rig score-context <trace-id>`.
It compares a trace's recorded `files_read` against compiled context rules and
retrieval triggers.

## Entropy Score

The drift score printed by `rig audit`. Lower is better.
...
A structured recommendation generated by `rig propose` from
...
The SQLite database and CLI (`rig`) that stores operational records
